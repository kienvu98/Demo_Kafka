...
import static com.microservices.demo.Constants.NA;
...
public class AnalyticsUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    private final AnalyticsUserDetailService analyticsUserDetailService;

    public AnalyticsUserJwtConverter(AnalyticsUserDetailService analyticsUserDetailService) {
        this.analyticsUserDetailService = analyticsUserDetailService;
    }
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        ...
        return Optional.ofNullable(
                analyticsUserDetailService.loadUserByUsername(jwt.getClaimAsString(USERNAME_CLAIM)))
                .map(userDetails -> {
    ...
    private Collection<GrantedAuthority> getAuthoritiesFromJwt(Jwt jwt) {
        return getCompineAuthorities(jwt).stream().map(SimpleGrantedAuthority::new).collect(Collectors.toList());
    }

    private Collection<String> getCompineAuthorities(Jwt jwt) {
