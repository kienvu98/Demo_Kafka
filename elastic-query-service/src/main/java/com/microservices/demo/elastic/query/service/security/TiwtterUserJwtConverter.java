...
public class TiwtterUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public TiwtterUserJwtConverter(TwitterQueryUserDetailService twitterQueryUserDetailService) {
        this.twitterQueryUserDetailService = twitterQueryUserDetailService;
    }

    // ham chuyen jwt thanh token
    @Override
    public AbstractAuthenticationToken convert(Jwt jwt) {
        Collection<GrantedAuthority> authoritiesFromJwt = getAuthoritiesFromJwt(jwt);
        return Optional.ofNullable(
                        twitterQueryUserDetailService.loadUserByUsername(jwt.getClaimAsString(USERNAME_CLAIM)))
                .map(userDetails -> {
    ...
    private Collection<String> getScope(Jwt jwt) {
        Object scopes = jwt.getClaims().get(SCOPE_CLAIM);
        if (scopes instanceof String) {
            return Arrays.stream(((String) scopes).split(SCOPE_SEPARATOR)).map(authority -> DEFAULT_SCOPE_PREFIX + authority.toUpperCase())
                    .collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
    ...
    private Collection<String> getCombinedAuthorities(Jwt jwt) {
        ...
        authorities.addAll(getScope(jwt));
        return authorities;
    }
